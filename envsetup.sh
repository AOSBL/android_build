...
function breakfast()
{
    target=$1
    local variant=$2
    AOSB_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/aosb/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the AOSB github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var AOSB_VERSION)
        ZIPFILE=aosb-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.aosb.device=$AOSB_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $AOSB_BUILD, run away!"
    fi
}
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/aosb/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
